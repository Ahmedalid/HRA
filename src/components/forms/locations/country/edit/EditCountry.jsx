...
import { useUpdateCountryMutation } from "../../../../../redux/slices/locations/locationsSlice";
...
export default function EditCountry({ setShowDialog }) {
  const [updateCountry, { isLoading }] = useUpdateCountryMutation();
  const currentCountry = useSelector(currentEditItem);
  ...
  const { register, handleSubmit } = useForm({
    ...
    defaultValues: currentCountry,
    mode: "all",
  });
  const onSubmit = async (data) => {
    if (isLoading) return;
    updateCountry({ id: currentCountry.id, _method: "put", ...data })
      .unwrap()
      .then((data) => {
        if (data.status) {
          setShowDialog(false);
